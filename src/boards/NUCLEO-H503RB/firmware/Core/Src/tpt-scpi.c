...
scpi_result_t TPT_AddPulse(scpi_t *context)
{
    ...
    /* Pre-compute BSRR value: even indices → positive, odd → negative */
    if (current_number_pulses % 2 == 0) {
        pulse_bsrr[current_number_pulses] =
            (NegativePulse_Pin << 16U) | PositivePulse_Pin;   /* neg off, pos on */
    } else {
        pulse_bsrr[current_number_pulses] =
            (PositivePulse_Pin << 16U) | NegativePulse_Pin;   /* pos off, neg on */
    }
